    ...
    $.fn.extend({
        lunbo: function(options) {
            var _that = this,
                ...
                click = null,
                elems = {
                    imgWidth: this.children('div').children('div').width(), //图片宽度
                    num: this.children('div').children('div').length //记录图片个数
                },
                defaults = {
                    speed: 500,
                    delay: 3000,
            ...
            init = function() {
                ...
                elems.innerDiv.append(elems.innerDiv.children('div').first().clone());
            ...
            start = function() { //参数为方向,因为btns只有两个，index后有0和1两个返回值
                ...
                if (!options.dir) { //点击左边按钮的时候
                    t = '+=' + wid;
                    if (elems.num + 2 == elems._index) {
                        var divLeft = _that.offset().left,
                            imgLeft = elems.innerDiv.children('div').last().offset().left;
                        elems._index = 1;
                        elems.innerDiv.css('left', '-' + (imgLeft - divLeft + 12.5) + "px");
                    };
            ...
            //         });
            //     }
            //     $(_that.children('em')[elems.num - 1]).addClass('show'); 
            // };
            //底部按钮点击
            click = function() {
